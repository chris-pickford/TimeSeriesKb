...
if OS == "Server":
    sys.path.append(
        os.path.join(
            os.path.expanduser("~:"), os.path.sep, "PYTHON", "Datascience Packages"
        )
    )
elif OS == "Local":
    sys.path.append(
        os.path.join(
            os.path.expanduser("~"), "Documents", "Coding", "Python", "Projects"
        )
    )

from DataCrane.config import config

config.googleServiceAccountPrivateKey = os.path.join(
    os.path.expanduser("~"),
    os.path.sep,
    "Users",
    "chrispickford",
...
class privateConfig(config):
    def __init__(self, OS, PROJECT_NAME):
        ...
        if OS == "Local":
            ...
            self.ROOT = os.path.join(
                os.path.expanduser("~:"), os.path.sep, "Chris Pickford", "Projects"
            )
            self.PROJECT_ROOT = os.path.join(self.ROOT, PROJECT_NAME)
            self.PACKAGE_ROOT = os.path.join(
                os.path.expanduser("~:"), os.path.sep, "Datascience Packages"
            )
    ...
    statusLog = {"OK ": "[__OK__]", "FAIL": "[_FAIL_]"}
    format = "%(asctime)s %(status)-8s %(separator)-3s %(message)s"

    dctLog = {
        "FAIL": {"status": "[ FAIL ]", "separator": " - "},
        "OK": {"status": "[      ]", "separator": " - "},
    }
